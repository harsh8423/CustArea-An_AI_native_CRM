                ...
                <div className={`flex items-center justify-between transition-all duration-500 ${scrolled ? 'h-14 md:h-16' : 'h-16'}`}>
                    ...
                    >
                        <div className="relative">
                            <img
                                src="/logo.png"
                                alt="CustArea"
                                className={`object-contain transition-all duration-500 group-hover:rotate-12 ${scrolled ? 'w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10' : 'w-8 h-8 sm:w-9 sm:h-9 md:w-11 md:h-11'}`}
                            />
                            <div className="absolute inset-0 bg-gradient-to-r from-blue-500/20 to-blue-600/20 blur-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 -z-10" />
                        </div>
                        <span className={`font-['Outfit'] font-bold transition-all duration-500 ${scrolled ? 'text-base sm:text-lg md:text-xl' : 'text-lg sm:text-xl md:text-2xl'}`}>
                    ...
                    <nav className="hidden lg:flex items-center gap-1">
                        {navLinks.map((link) => (
                            ...
                            >
                                ...
                                {/* Animated underline */}
                                <span className={`absolute left-1/2 -translate-x-1/2 h-0.5 bg-[#1E4A8D] rounded-full group-hover:w-3/4 transition-all duration-300 ${scrolled ? 'bottom-1 w-0' : 'bottom-0 w-0'
                                    }`} />
                            </a>
                        ))}
                    </nav>
                    ...
                    <div className="hidden lg:flex items-center">
                        ...
                        >
                            ...
                            <span className="relative flex items-center gap-2 font-['Inter']">
                        ...
                        <nav className="flex flex-col gap-1 pt-2">
                            {navLinks.map((link) => (
                                <a
                                    ...
                                    href={link.href}
                                    onClick={(e) => handleNavClick(e, link.href)}
                                    className="py-3 px-4 text-slate-700 hover:text-[#1E4A8D] hover:bg-slate-50 rounded-full transition-all duration-200 font-medium text-sm min-h-[44px] flex items-center"
                                >
                                    {link.label}
                                </a>
                            ))}
